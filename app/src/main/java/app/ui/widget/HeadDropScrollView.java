...
/**
 * Created by jxy on 2018/3/31.
 */

public class HeadDropScrollView extends NestedScrollView {
    public HeadDropScrollView(Context context) {
        super(context);
    }

    public HeadDropScrollView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public HeadDropScrollView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    /**
     * 自定义控件中 子布局映射完成 调用此方法，即当子控件映射成xml文件
     */
    @Override
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            case MotionEvent.ACTION_MOVE:
                //记录按下点的位置
                if (!mScaling) {
                    if (getScrollY() == 0) {    //当前view的左上角相对于父View左上角y轴偏移量
                        y = ev.getY();          //滑动到顶部时，记录位置   点击的点距离空间顶端
                    } else {
                        break;
                    }
                }
