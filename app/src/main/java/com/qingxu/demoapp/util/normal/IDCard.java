...
public class IDCard {
	public static IDCardResult IDCardValidate(String IDStr) throws ParseException {
		String tipInfo = "该身份证有效！";// 记录错误信息
		String Ai = "";
		// 判断号码的长度 15位或18位
		if (IDStr.length() != 15 && IDStr.length() != 18) {
			tipInfo = "身份证号码长度应该为15位或18位。";
			return IDCardResult.build(0, tipInfo);
		}
		...
		return IDCardResult.build(1, tipInfo);
	}
...
//	}
	...
	public static class IDCardResult {
		...
		public static IDCardResult build(int code, String tipInfo){
			return new IDCardResult(code,tipInfo);
		}

		private IDCardResult(int code, String tipInfo) {
			this.code = code;
