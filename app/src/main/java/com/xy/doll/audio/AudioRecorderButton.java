...
//                + "/kairui/VoiceCache";                             // 此处需要判断是否有存储卡(外存)
    ...
    private void changeState(int state) {
        if (mCurState != state) {
            ...
            switch (state) {
                case STATE_NORMAL:
                    setBackgroundResource(R.drawable.emoji_1);
//                    setText(R.string.push_to_speak);
                    setText("讲话");
                    break;

                case STATE_RECORDERING:
                    setBackgroundResource(R.drawable.emoji_2);
//                    setText(R.string.release_to_send);
                ...
                case STATE_WANT_TO_CALCEL:
                    setBackgroundResource(R.drawable.emoji_3);
    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(android.os.Message msg) {
            switch (msg.what) {
                ...
                case MSG_CURRENT_TIME:          //265:mHandler.sendEmptyMessage(MSG_VOICE_CHANGE); 更新当前录音秒数
//                    audioDialogManage.updateCurTime(TimeUtils.countDown(mTime));
                    audioDialogManage.updateCurTime(DateUtils.getDateToString(mTime));
                    break;
