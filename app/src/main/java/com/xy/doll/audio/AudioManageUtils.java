...
public class AudioManageUtils {
    ...
    private static AudioManageUtils mInstance;

    private boolean isPrepared; // 标识MediaRecorder准备完毕

    private AudioManageUtils(String dir) {
        mDir = dir;
    }
    ...
    public static AudioManageUtils getInstance(String dir) {
        if (mInstance == null) {
            synchronized (AudioManageUtils.class) {   // 同步
                if (mInstance == null) {
                    mInstance = new AudioManageUtils(dir);
                }
            }
        }
